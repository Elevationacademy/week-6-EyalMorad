...
Book.find(
  {rating:{"$lt":5}}).sort({author:-1}).exec(function(err, books) {
    //console.log(books)
  })
...
Person.find(
  ({height:{"$gt":180}, salary:{"$gt":30000}}),function(err,people){
    //console.log(people)    //just to check
  }
)
...
Person.find(
  { $or:[ {height:{"$gt":180}},{salary:{"$gt":30000}}]}, function(err, people) {
    //console.log(people)    //just a check
  }
)
...
Person.find().and([ //complex a little bit ??
